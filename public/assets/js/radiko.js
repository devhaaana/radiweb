...
async function loadStationList(filterAreaId = null) {
	...
	const countElem = document.querySelector('.station-count-container');

	if (countElem) {
		const target = container.childElementCount;
	    let current = 0;
		const duration = 800;
	    ...
	    function animateCount(now) {
	        ...
	        // countElem.textContent = `${current} stations`;
			const numberElem = document.getElementById('station-count-number');
	        if (numberElem) {
	...
	// if (countElem) {
	// 	const targetCount = container.childElementCount;
	//     let current = 0;
	//     const speed = Math.max(10, Math.floor(500 / targetCount)); // animation speed
...
if (reloadButton) {
	reloadButton.addEventListener("click", () => {
		...
		geoLayer.setStyle({ fillColor: '#f2f2f2', fillOpacity: 0.7 });
